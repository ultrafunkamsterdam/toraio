...
class Pool(object):
        ...
        if instance is not None:
            ...
            if instance.process:
                if instance.process.poll() is None:
                    logging.getLogger(__name__).debug('already initialized. returning running instance')
                    return instance
    ...
    def stop(cls, datadir=None, message=None):
        if cls.__instance__.process:
            logging.getLogger(__name__).debug("stopping proxy pool \ninstance: %s,  datadir: %s , message: :%s" % (
    ...
    async def open_connection(self, host, port, remote_resolve=True, **kw):
        ...
        return await aiosocks.open_connection(proxy, None, dst=(host,port), remote_resolve=remote_resolve, **kw)
...
def test():
    ...
    import logging
    from aiotor import ClientSession
